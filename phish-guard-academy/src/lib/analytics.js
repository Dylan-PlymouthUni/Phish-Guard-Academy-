...
export function logChallengeAnswer(category, correct) {
    ...
    if (correct)
        store.challenges.byCategory[cat].correct += 1;
    saveStore(store);
}
export function logLessonEvent(topicId, event) {
    const store = loadStore();
...
export function getAnalyticsSummary() {
    ...
    const categoryBreakdown = Object.entries(byCategory).map(([cat, { total, correct }]) => ({
        category: cat,
        total,
        correct,
